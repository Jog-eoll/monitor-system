 ...
 * AUTHENTICATED --[等待用户选择进程]--> PROCESS_SELECTION_REQUIRED
 * PROCESS_SELECTION_REQUIRED --[进程绑定成功]--> AUTHENTICATED
 * AUTHENTICATED --[通道建立]--> CHANNEL_ACTIVE
 * 任何状态 --[UKey拔出]--> IDLE（触发断开/清理）
 */
@Slf4j
@Component
public class UkeyLifecycleManagerImpl implements UkeyLifecycleManager {

    /**
     * UKey生命周期状态枚举
     */
    private enum LegacyStateUnused {
        IDLE("空闲，等待UKey插入"),
        UKEY_DETECTED("UKey已检测到，读取证书中"),
        UKEY_VALIDATED("UKey本地校验通过"),
        AUTHENTICATING("正在与管控平台双向认证"),
        AUTHENTICATED("认证成功，可进行业务"),
        PROCESS_SELECTION_REQUIRED("认证成功，等待选择 Sigma 程序"),
        CHANNEL_ACTIVE("转发通道已建立"),
        ERROR("异常状态");

        private final String description;

        LegacyStateUnused(String description) {
     ...
     * 状态变更监听器
     */
    private interface LegacyStateChangeListenerUnused {
    ...
    /**
     * 认证成功后等待用户选择 Sigma 程序。
     */
    public synchronized boolean onProcessSelectionRequired() {
        if (currentState != State.AUTHENTICATED && currentState != State.CHANNEL_ACTIVE) {
            log.warn("无法进入PROCESS_SELECTION_REQUIRED状态，当前状态: {}", currentState);
            return false;
        }

        transition(State.PROCESS_SELECTION_REQUIRED, "等待用户选择 Sigma 程序");
        return true;
    }

    /**
     * 用户选择并绑定进程成功后，回到认证完成状态。
     */
    public synchronized boolean onProcessBound() {
        if (currentState != State.PROCESS_SELECTION_REQUIRED && currentState != State.AUTHENTICATED) {
            log.warn("无法完成进程绑定状态转换，当前状态: {}", currentState);
            return false;
        }

        transition(State.AUTHENTICATED, "Sigma 程序已绑定");
        return true;
    }
     ...
     * 状态概要
     */
    private static class LegacyStatusOverviewUnused {
        ...
        LegacyStatusOverviewUnused(State state, String stateDescription, String certSerialNo,
