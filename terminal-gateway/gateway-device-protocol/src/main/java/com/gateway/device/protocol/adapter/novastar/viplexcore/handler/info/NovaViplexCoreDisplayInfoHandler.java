 ...
 * 显示信息查询处理器 —— 获取屏幕显示参数。
 *
 * <p>SDK: {@code nvGetDisplayInfoAsync}</p>
 */
@Slf4j
public class NovaViplexCoreDisplayInfoHandler extends AbstractNovaViplexCoreHandler<EmptyParams> {

    public NovaViplexCoreDisplayInfoHandler(
    ...
    public DeviceCapability<EmptyParams> capability() {
        return NovaViplexCoreCapability.DISPLAY_INFO_GET;
    }
    ...
    private CommandResult executeQuery(String sn) {
        ...
        try {
            ...
            ViplexResponse resp = channel().execute(
                    SdkFunction.NV_GET_DISPLAY_INFO_ASYNC, json, getTimeout());
            long cost = System.currentTimeMillis() - start;
            if (resp.isTimeout()) {
                return CommandResult.builder()
                        .success(false).code(StandardErrorCode.TIMEOUT)
                        .message("获取显示信息超时").costMillis(cost).build();
            }
            if (!resp.isSuccess()) {
                log.warn("获取显示信息失败 SN={} code={} data={}", sn, resp.getCode(), resp.getData());
                return CommandResult.builder()
                        .success(false).code(StandardErrorCode.PROTOCOL_ERROR)
                        .message(String.format("获取显示信息失败: %s, %s", ViplexErrorCode.describe(resp.getCode()), resp.getData()))
                        .costMillis(cost).build();
            }
            JsonNode data = resp.dataAsJson();
            log.info("获取显示信息成功 SN={}", sn);
            return CommandResult.builder()
                    .success(true).code(StandardErrorCode.SUCCESS)
                    .data(data).costMillis(cost).build();
        } catch (Exception e) {
            long cost = System.currentTimeMillis() - start;
            log.error("获取显示信息异常 SN={}", sn, e);
            return CommandResult.builder()
                    .success(false).code(StandardErrorCode.SYSTEM_ERROR)
                    .message(String.format("获取显示信息异常: %s", e.getMessage())).costMillis(cost).build();
        }
    }
}
