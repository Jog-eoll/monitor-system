...
/**
 * ColorLight 唤醒控制 —— POST /api/action {"command":"wakeup"}。
 */
@Slf4j
public class ColorLightWakeupHandler extends AbstractColorLightHttpHandler<EmptyParams> {

    public ColorLightWakeupHandler(DeviceTransport transport,
    ...
    public DeviceCapability<EmptyParams> capability() {
        return CommonDeviceCapability.POWER_CONTROL_WAKEUP;
    }

    @Override
    public CommandResult execute(DeviceContext device, EmptyParams params) {
        ActionPayload payload = ActionPayload.builder().command("wakeup").build();
        ...
        if (resp == null || resp.getStatusCode() != HttpURLConnection.HTTP_OK) {
            return failureResult("CL_WAKEUP_FAIL", "Failed to set screen wakeup");
        }
