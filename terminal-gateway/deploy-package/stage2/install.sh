...
check_delivery_files() {
  ...
  # 检查 Docker 镜像
  local image="gateway-udp-proxy:1.0"
  if ! docker image inspect "$image" &>/dev/null; then
    if [[ -f "$DEPLOY_DIR/images/terminal-gateway.tar.gz" ]]; then
      log_info "加载 Docker 镜像..."
      docker load -i "$DEPLOY_DIR/images/terminal-gateway.tar.gz"
...
VAUTH_SVAC_MODE='${VAUTH_SVAC_MODE:-true}'
...
DEVICE_REMARK='${DEVICE_REMARK:-terminal decrypt gateway}'

# 截图配置
SNAPSHOT_ENABLED=${SNAPSHOT_ENABLED:-false}
SNAPSHOT_INTERVAL_SECONDS=${SNAPSHOT_INTERVAL_SECONDS:-10}
SNAPSHOT_TIMEOUT_MS=${SNAPSHOT_TIMEOUT_MS:-5000}
...
# 镜像版本
GATEWAY_VERSION=${GATEWAY_VERSION:-1.0}
EOF
...
start_services() {
  ...
  # 启动 MySQL 数据库
  log_info "  启动 MySQL..."
  $COMPOSE_CMD -f "$COMPOSE_FILE" up -d terminal-gateway-mysql
  sleep 10  # 等待 MySQL 初始化

  # 启动解密网关
  log_info "  启动解密网关..."
  $COMPOSE_CMD -f "$COMPOSE_FILE" up -d terminal-gateway
...
check_health() {
  ...
  while (( mysql_retries < mysql_max_retries )); do
    if docker inspect --format='{{.State.Status}}' terminal-gateway-mysql 2>/dev/null | grep -q "running"; then
  ...
  while (( retries < max_retries )); do
    if docker inspect --format='{{.State.Status}}' terminal-gateway-standalone 2>/dev/null | grep -q "running"; then
      # 容器运行中，检查应用是否就绪
      if curl -sf "http://127.0.0.1:${GATEWAY_HTTP_PORT:-8093}/udp-proxy/rules/running" &>/dev/null; then
        log_success "解密网关健康检查通过"
        return 0
      fi
    fi
  ...
  log_error "健康检查超时，请查看日志: docker logs terminal-gateway-standalone"
  return 1
}
...
================================================================
  terminal-gateway 部署账户信息
  生成时间: $(date '+%Y-%m-%d %H:%M:%S')
================================================================

一、服务访问地址

  解密网关 HTTP:    http://$(hostname -I | awk '{print $1}'):${GATEWAY_HTTP_PORT:-8093}
...
四、常用命令
  ...
  查看状态:         docker ps | grep terminal-gateway
  查看日志:         docker logs -f terminal-gateway-standalone
  重启服务:         docker restart terminal-gateway-standalone
  停止服务:         docker stop terminal-gateway-standalone
...
main() {
  ...
  echo ""
  echo "================================================"
  log_success "terminal-gateway 部署完成！"
  echo "================================================"
  echo ""
  log_info "请查看 account.txt 获取访问信息"
  log_info "查看日志: docker logs -f terminal-gateway-standalone"
  echo ""
}
