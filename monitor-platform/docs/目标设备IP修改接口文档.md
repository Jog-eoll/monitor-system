...
本文档供前端页面接入“修改目标设备 IP”功能使用。当前后端实现位于 `monitor-platform-forward`，通过 MQTT 向目标网关设备下发系统网络修改命令。

## 1. 接口边界

当前接口支持的目标设备必须满足：

- 设备已接入平台 MQTT 链路，且平台设备编号可作为 `targetDeviceId` 使用。
- 设备侧运行 `gateway-udp-proxy` MQTT Agent。
- 设备侧已启用 `SYSTEM_NETWORK_CHANGE_ENABLED=true`。
- 设备侧允许修改的网卡包含在 `SYSTEM_NETWORK_CHANGE_ALLOWED_INTERFACES` 中。

当前版本不支持通过接口完成“删除旧 IP 并持久化正式割接”。设备侧代码会拒绝 `persist=true`。因此前端应将该能力展示为“下发目标 IP 变更命令/安全变更”，不要承诺一定完成系统持久化替换。
...
| `targetDeviceId` | string | 是 | 目标设备编号，通用字段，不限定网关设备 |
| `gatewayDeviceId` | string | 否 | 旧字段，仅用于兼容历史调用；新页面不要使用 |
| `changeId` | string | 是 | 与修改 IP 接口保持一致 |
| `operator` | string | 否 | 操作人账号 |
...
- 如产品需要“正式把旧 IP 替换为新 IP”，需要后端新增删除旧 IP、持久化网络配置、回滚和状态查询能力。
