...
public class RemoteUpgradeReplyServiceImpl implements RemoteUpgradeReplyService {
    ...
    public void handleReply(DeviceMqttCommand command, MqttReplyMessage reply) {
        ...
        RemoteUpgradeTaskDevice taskDevice = findTaskDevice(command);
        if (taskDevice == null) {
    ...
    @Override
    public void handleTimeout(DeviceMqttCommand command) {
        if (command == null || !RemoteUpgradeConstants.COMMAND_REMOTE_UPGRADE.equalsIgnoreCase(command.getCommand())) {
            return;
        }
        RemoteUpgradeTaskDevice taskDevice = findTaskDevice(command);
        if (taskDevice == null) {
            log.warn("[远程升级] 未找到 MQTT 超时对应任务明细: messageId={}", command.getMessageId());
            return;
        }
        LocalDateTime now = LocalDateTime.now();
        taskDevice.setStatus(RemoteUpgradeConstants.DEVICE_STATUS_TIMEOUT);
        taskDevice.setStage("TIMEOUT");
        taskDevice.setAckTime(command.getAckTime());
        taskDevice.setFinishTime(now);
        taskDevice.setErrorMessage(command.getErrorMessage());
        taskDevice.setUpdateTime(now);
        taskDeviceMapper.updateById(taskDevice);
        refreshTaskSummary(taskDevice.getTaskId());
    }

    private RemoteUpgradeTaskDevice findTaskDevice(DeviceMqttCommand command) {
        ...
        wrapper.eq(RemoteUpgradeTaskDevice::getMqttMessageId, command.getMessageId())
                .last("LIMIT 1");
        return taskDeviceMapper.selectOne(wrapper);
    }
