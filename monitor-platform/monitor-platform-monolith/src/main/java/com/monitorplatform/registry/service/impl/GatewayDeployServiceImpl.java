...
public class GatewayDeployServiceImpl implements GatewayDeployService {
    ...
    public GatewayRegisterResponse register(GatewayRegisterRequest request) {
        ...
        log.info("gateway registered in monolith: deviceId={}, ip={}, port={}, role={}",
    ...
    public GatewaySelfTestResponse reportSelfTest(String deviceId, GatewaySelfTestRequest request) {
        ...
        log.info("gateway self-test reported in monolith: deviceId={}, status={}", normalizedDeviceId, overallStatus);
        return response;
    }
    ...
    private CertBindingResult validateAndBindCertificate(GatewayRegisterRequest request) {
        CertValidateRequestDTO validateRequest = new CertValidateRequestDTO();
        ...
        CertValidateResponseDTO validateResponse = ukeyCertificateService.validateCertificate(validateRequest);
        if (validateResponse == null || !validateResponse.isValid()) {
            String message = validateResponse == null ? "certificate validation failed" : validateResponse.getMessage();
            throw new GatewayDeployException(400, message);
        }

        String actualCertSerialNo = firstNotBlank(validateResponse.getCertSerialNo(), request.getCertSerialNo());
        boolean bound = ukeyCertificateService.bindToClient(actualCertSerialNo, request.getDeviceId());
        if (!bound) {
            throw new GatewayDeployException(404, "certificate bind failed: certificate not found");
        }

        CertBindingResult result = new CertBindingResult();
        result.setValid(true);
        result.setBound(true);
        result.setCertSerialNo(actualCertSerialNo);
        result.setBoundClientId(firstNotBlank(validateResponse.getBoundClientId(), request.getDeviceId()));
        return result;
    }
