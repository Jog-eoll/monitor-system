...
// 手动设置核心字段
   ...
   public TaskChainVO getChainTree(Long chainId){
    ...
    /**
     * 校验设备是否存在于unified_device_info表中（通过本地 UnifiedDeviceService 直接查询）
     */
    private void checkDeviceExists(String deviceId, String deviceType) {
        if (unifiedDeviceService == null) {
            log.warn("UnifiedDeviceService未配置，跳过设备存在性校验");
            return;
        }
    ...
    private Map<String, Object> probeDeviceStatus(TaskChainNode node) {
        ...
        if (unifiedDeviceService == null) {
            // UnifiedDeviceService未配置，回退到节点存储状态
        ...
        try {
            DeviceDetailDTO detail = unifiedDeviceService.getDeviceDetail(node.getDeviceType(), node.getDeviceId());

            if (detail != null && detail.getDeviceId() != null) {
                String deviceStatus = detail.getStatus();
                ...
                // 将 DeviceDetailDTO 转为 Map 供 buildUnhealthyNodeInfo 使用
                Map<String, Object> deviceData = new HashMap<>();
                deviceData.put("status", detail.getStatus());
                deviceData.put("alarmCount", detail.getAlarmCount());
                deviceData.put("lastAlarmTime", detail.getLastAlarmTime());
                deviceData.put("lastAlarmType", detail.getLastAlarmType());
                deviceData.put("remark", detail.getRemark());
