...
public class VAuthServerController {
    ...
    public Result<?> handleAuthRequest(@RequestBody AuthExchangeRequestDTO request) {
        log.info("[VAuthServer][HTTP] 收到认证请求: authId={}, requestDataLen={}, hasClientCert={}",
                request.getAuthId(), safeLength(request.getRequestData()), hasText(request.getClientCert()));
        try {
            String reply = authServerService.handleAuthRequest(request.getAuthId(), request.getRequestData());
            log.info("[VAuthServer][HTTP] 认证请求处理成功: authId={}, replyLen={}",
                    request.getAuthId(), safeLength(reply));
            return Result.data(reply);
        } catch (Exception e) {
            log.error("[VAuthServer][HTTP] 处理认证请求异常: authId={}, requestDataLen={}",
                    request.getAuthId(), safeLength(request.getRequestData()), e);
    ...
    public Result<?> handleAuthVerify(@RequestBody AuthExchangeRequestDTO request) {
        log.info("[VAuthServer][HTTP] 收到认证验证请求: authId={}, requestDataLen={}, clientCertLen={}",
                request.getAuthId(), safeLength(request.getRequestData()), safeLength(request.getClientCert()));
        try {
            String reply = authServerService.handleAuthVerify(request.getAuthId(), request.getClientCert(), request.getRequestData());
            log.info("[VAuthServer][HTTP] 认证验证处理成功: authId={}, replyLen={}",
                    request.getAuthId(), safeLength(reply));
            return Result.data(reply);
        } catch (Exception e) {
            log.error("[VAuthServer][HTTP] 处理认证验证异常: authId={}, requestDataLen={}, clientCertLen={}",
                    request.getAuthId(), safeLength(request.getRequestData()), safeLength(request.getClientCert()), e);
            return Result.fail(e.getMessage());
        }
    }
    ...
    public Result<?> handleAuthError(@RequestBody Map<String, String> body) {
        ...
        log.info("[VAuthServer][HTTP] 收到认证错误上报: errorInfoLen={}", safeLength(errorInfo));
        try {
