    ...
    public MdnsRegisterResult registerScannedDevices() {
        ...
        for (DiscoveredService service : services) {
            ...
            try {
                ...
                CommonServiceResponseVO<Void> response = deviceFeignClient.autoRegisterDevice(dto);
                if (response != null && response.getCode() != null && response.getCode() == SUCCESS_CODE) {
                    item.setResult("REGISTERED");
                    item.setMessage("registered");
                    result.incrementRegistered();
                } else {
                    item.setResult("FAILED");
                    item.setMessage(response == null ? "monitor-device returned null" : response.getMsg());
